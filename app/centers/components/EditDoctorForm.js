...
import { FiUser, FiPhone, FiMail, FiMapPin, FiHome, FiCamera, FiX, FiCheck } from 'react-icons/fi';
...
const EditDoctorForm = ({ doctor, onSave, onCancel, cities, isLoading, error }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Ensure we're passing the complete doctor data including _id
      const doctorDataToSave = {
        ...formData,
        _id: doctor._id
      };
      
      await onSave(doctorDataToSave);
          ...
          <div className="lg:col-span-1">
            ...
            {formData.photo ? (
              <div className="flex flex-col items-center">
                ...
                <div 
                  {...getRootProps()} 
                  className="mt-2 text-sm text-blue-600 hover:text-blue-800 cursor-pointer flex items-center"
                >
              ...
              <div 
                ...
                className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer hover:border-blue-500 transition min-h-40 flex items-center justify-center 
                  ${isProcessingImage ? 'opacity-50 border-gray-300 bg-gray-50' : 'border-gray-300 bg-gray-50'}`}
              >
